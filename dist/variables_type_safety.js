...
let firstName = myName.slice(0, myName.indexOf(" ") + 1);
console.log(firstName);
// Boolean
let isValid = true;
const isAdult = false;
let idVerified = isValid && isAdult;
// Any - The God Father
// By using any you are letting ts of the type safety now it works same like js
let ground = "Football Ground";
ground = 78;
...
let email = "tayyabpasha1918@gmail.com"; // here as we annotated its as unknown but we initialized it with string now we can't use string specific methods on email as it's type is unknown to do that we to first explicitly tell ts it's original type.
let tls = email.slice(email.indexOf("@")); // not allowed as email is unknown
